...
class ProgramTypeModel
{
    ...
    public function create(int $schoolId, string $name): int
    {
        ...
        $stmt = $this->db->prepare("
            INSERT INTO store_program_types (school_id, name)
            VALUES (?, ?)
        ");
    ...
    public function update(int $id, int $schoolId, string $name): bool
    {
        if ($this->exists($schoolId, $name, $id)) {
            throw new Exception("Duplicate program type name");
        }

        $stmt = $this->db->prepare("
            UPDATE store_program_types
            SET name = ?
    ...
    {
        $sql = "
            SELECT id FROM store_program_types
            WHERE school_id = ?
            AND name = ?
            AND is_active = 1
        ";
